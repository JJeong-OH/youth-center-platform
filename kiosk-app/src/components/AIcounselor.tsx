...
import { YOUTH_FACILITIES } from '../constants/constants';
...
export const AICounselor: React.FC = () => {
  ...
  const [bookingRequest, setBookingRequest] = useState<{ facilityId: string, userName: string } | null>(null);
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    try {
      ...
      if (response.functionCalls && response.functionCalls.length > 0) {
        ...
        if (fc.name === 'bookFacility' && fc.args.facilityId && fc.args.userName) {
             setBookingRequest({ facilityId: fc.args.facilityId, userName: fc.args.userName });
        }
      }
  ...
  const confirmBooking = () => {
    ...
    const facility = YOUTH_FACILITIES.find(f => f.id === bookingRequest.facilityId);
    const systemMessage: ChatMessage = {
        role: 'system',
        text: `${bookingRequest.userName}님의 '${facility?.name || '시설'}' 예약이 완료되었습니다. (예약일: 오늘)`
    };
    ...
    <div className="flex flex-col h-full">
      ...
      {bookingRequest && (
        <FacilityBookingModal
            facilityName={YOUTH_FACILITIES.find(f => f.id === bookingRequest.facilityId)?.name || ''}
