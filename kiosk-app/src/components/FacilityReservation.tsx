      ...
      <div className="bg-white/70 backdrop-blur-xl rounded-2xl shadow-lg border border-white/30 p-5 overflow-hidden transition-all duration-300">
        <div className="flex flex-col sm:flex-row gap-5">
          ...
          <div className="flex flex-col flex-grow">
            ...
            <p className="text-slate-600 mt-2 flex-grow text-sm">{facility.description}</p>
            {!isPast && (
              <button
                onClick={() => setIsExpanded(!isExpanded)}
                className="mt-4 w-full sm:w-auto self-start sm:self-end px-6 py-2 rounded-lg bg-indigo-500 text-white font-semibold hover:bg-indigo-600 transition-colors"
              >
                {isExpanded ? '시간 선택 닫기' : '예약 시간 선택'}
              </button>
            )}
          </div>
        </div>
          ...
          <div className="mt-4 pt-4 border-t border-slate-200/80">
                ...
                if (isBooked) {
                  return (
                    <button
                      key={timeSlot}
                      disabled
                      className="p-3 rounded-lg text-center text-sm font-semibold bg-gray-400 text-gray-100 cursor-not-allowed opacity-70 border-2 border-gray-500"
                    >
                      <div className="line-through">{timeSlot.split('-')[0]}</div>
                      <div className="text-xs mt-1 text-red-700 font-bold">예약됨</div>
                    </button>
                  );
                }
            ...
            {selectedTimeSlots.length > 0 && (
              <button
                ...
                className="mt-4 w-full px-6 py-3 rounded-lg bg-green-500 text-white font-bold hover:bg-green-600 transition-colors shadow-lg flex items-center justify-center gap-2"
              >
                <CheckCircleIcon className="w-5 h-5" />
                선택한 {selectedTimeSlots.length}개 시간대 예약하기
              </button>
            )}
          </div>
        )}
  ...
  onRefreshBookings: () => void 
    ...
    return (
        <div style={{ 
            ...
            height: '100%',
            overflow: 'hidden'  // ✅ 부모 컨테이너 스크롤 방지
        }}>
            {/* 날짜 선택 - 고정 */}
            <div className="bg-white/70 backdrop-blur-xl rounded-2xl shadow-lg border border-white/30 p-4 mb-4 flex-shrink-0">
                <label htmlFor="date-picker" className="flex items-center gap-2 text-lg font-bold text-slate-700 mb-2">
                ...
                <input
                    ...
                    className="w-full bg-white/70 rounded-lg p-3 border border-slate-300 focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                />
            </div>

            {/* 에러 메시지 - 고정 */}
            {errorMessage && (
              <div className="bg-red-100 border border-red-300 text-red-800 px-4 py-3 rounded-xl mb-4 flex items-center gap-3 animate-pulse flex-shrink-0">
            ...
            <div style={{ 
                ...
                overflowY: 'auto',
                paddingRight: '8px',
                paddingBottom: '140px'  // 뒤로가기 버튼 공간
            }}>
                <div style={{ 
                    display: 'flex', 
                    flexDirection: 'column', 
    ...
    return (
      <div className="flex flex-col h-full">
        ...
        <div className="flex items-center justify-between mb-4 px-2">
          <h2 className="text-2xl md:text-3xl font-bold text-indigo-600">시설 예약</h2>
          <div className="flex items-center p-1 rounded-xl bg-white/70 border border-white/30 shadow-sm">
              <button 
                  onClick={() => setViewMode('new')}
                  className={`px-4 py-2 text-sm font-semibold rounded-lg transition-colors duration-200 flex items-center gap-2 ${viewMode === 'new' ? 'bg-indigo-500 text-white shadow' : 'text-slate-600 hover:bg-white/50'}`}
              >
                  신규 예약
              </button>
              <button
